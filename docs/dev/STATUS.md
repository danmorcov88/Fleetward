...
**Current position: Phase A, slice A3 — sandbox provider.**
Foundation and slices A1 and A2 are complete.

Brief: [`docs/dev/slices/A3-sandbox-provider.md`](slices/A3-sandbox-provider.md).
...
| **A3** | `SandboxProvider` (Docker), teardown guaranteed on every path including panic | A test starts and destroys a container; no orphans survive | 🔨 next |
