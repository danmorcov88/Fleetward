// Package mysql implements the Fleetward engine plugin for MySQL / MariaDB.
//
...
const EngineType = "mysql"

// Plugin is the MySQL / MariaDB engine plugin.
//
...
func (p *Plugin) Capabilities(context.Context) (*fwv1.Capabilities, error) {
	return &fwv1.Capabilities{
		...
		EngineDisplayName:       "MySQL / MariaDB",
		PluginVersion:           version.Version,
		ContractVersion:         version.ContractVersion,
		SupportedEngineVersions: []string{">=8.0"},

		// Stage 3 turns these on alongside the implementations behind them:
		//   BackupMethods: xtrabackup as the physical method, mysqldump as the logical fallback
		//   that ships first for MVP simplicity.
		//   PrincipalModel PRINCIPAL_MODEL_USERS_AND_GRANTS.
		//   SandboxTemplate (mysql image, mysqladmin ping readiness probe).
